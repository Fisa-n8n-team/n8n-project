...
`Dynamic Programming`, `Graph Search`, `Greedy` 등 Solved.ac의 **알고리즘 Tag**를 기반으로 사용자가 취약한 유형을 선택하여 집중 학습할 수 있도록 쿼리 로직을 고도화할 예정입니다.

**2. 난이도 밸런싱 및 중복 방지**

현재는 특정 범위 내에서 무작위로 추출하다 보니, 비슷한 난이도의 문제가 중복되거나 특정 티어에 편중될 가능성이 있습니다.

따라서, 난이도가 겹치지 않도록 계단식으로 문제를 배분하거나, 최근 푼 문제의 난이도를 분석해 점진적으로 난이도를 높이는 추천 시스템을 도입할 계획입니다.
